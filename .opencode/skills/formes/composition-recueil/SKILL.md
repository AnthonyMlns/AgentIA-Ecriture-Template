---
name: composition-recueil
description: Techniques de composition d'un recueil multi-textes — anaphore de section, écho lexical, progression en croix, condensation.
maturité: testé
---

# Skill — Composition de recueil

> Techniques de composition pour les recueils de poésie, de nouvelles ou de textes courts. Organiser, lier et équilibrer un ensemble de textes.
> Issu du dédoublement de `poesie-contemporaine` (REX transversal v7.0, 07/06/2026). Les 7 outils étaient auparavant dans la sous-catégorie « Structure de recueil » de `poesie-contemporaine`.
> Mis à jour le 07/06/2026 — création par dédoublement. Maturité `ancré` héritée de `poesie-contemporaine`.
> Mis à jour le 15/06/2026 — ajout depuis `poesie-contemporaine` (audit lisibilité) : 1 principe « Arc de recueil multi-parallèle » + 2 outils « Arc de section par contraste de chute », « Réserve lyrique ». 9→12 outils.

---

## Principes

- **Le recueil est un objet autonome** : les poèmes/récits ne sont pas une collection — ils forment un arc. L'ordre n'est pas interchangeable.
- **Un fil rouge par section** : un mot, un motif, un registre qui traverse la section sans être martelé. Les liens sont souterrains.
- **Variété des formes** : alterner les longueurs, les registres, les densités. Un recueil uniforme fatigue.
- **La section finale n'est pas une conclusion** : elle est le point de convergence de tous les arcs. Elle ouvre plutôt qu'elle ne ferme.
- **Arc de recueil multi-parallèle** : un recueil de plusieurs sections peut construire des arcs parallèles sur 3+ dimensions (voix, motifs, lumière, doute, chutes). Chaque arc suit sa propre progression mais tous convergent vers la section finale.
- **Transverse** : ce skill s'applique à tout recueil multi-textes — poésie, nouvelles, textes mobiles, carnets. Les outils sont adaptables au genre.

---

## Boîte à outils du compositeur de recueil

Techniques disponibles, jamais obligatoires — le compositeur pioche selon les besoins du recueil. La rareté fait la puissance : un outil utilisé partout n'est plus nulle part.
...
- **Hybridation générique** : un recueil peut inclure une préface-manifeste en prose dense, des passages théoriques intercalés, ou des formes hybrides (poème en prose, fragment théâtral, note de carnet). L'hétérogénéité formelle fait partie du propos si elle est construite.
...
- **Arc de section par contraste de chute** : dans un recueil multi-sections, les chutes des sections peuvent se répondre par contraste pour créer un arc émotionnel. Une section peut fermer absolument (mot seul, silence) tandis que la suivante ouvre (adresse suspendue, question sans réponse). Le contraste entre fermeture et ouverture donne au recueil une respiration.
...
- ✗ Ordre interchangeable — si on peut déplacer les textes sans perte de sens, le recueil n'a pas d'arc
- ✗ Uniformité des longueurs — alterner les formats
- ✗ Saturation des motifs signatures (eau, feu, lumière) répétés sans progression
- ✗ Chute rhétorique en fin de recueil — préférer une image qui continue de travailler
...
- Avec `nouvelle-litteraire` : les techniques de progression en croix, d'écho lexical et de pièce respiration sont directement transposables à un recueil de nouvelles.
- Avec `ecriture-carnet-journal` : la condensation de recueil et l'hybridation générique sont compatibles avec la fragmentation du carnet.
- Avec `ecriture-hybride` : compatibilité naturelle — les formes hybrides sont au cœur des deux skills.
...
- **Skills actifs** : composition-recueil + poesie-contemporaine (empilage)
- **Ce qui a fonctionné** : l'arc pronominal (absence→je→tu→nous) a donné sa structure au recueil. La section 02 comme solo de 29 vers a été validée comme « pièce respiration étendue ». La convergence des motifs en section 03 a été reconnue par l'éditeur (8,8/10, 0 REFUS).
...
| 15/06 | Carnets Classiques | +2 outils (Arc gestuel, Clôture réponse) | 7→9 outils |
