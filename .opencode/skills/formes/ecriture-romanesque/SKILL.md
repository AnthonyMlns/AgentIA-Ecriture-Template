...
- **Structure d'enquête en entonnoir** : organiser les entretiens en progression de profondeur décroissante — chaque témoin est plus proche du secret que le précédent. Structure type : (1) **Témoin officiel** (fonctionnaire, garde, notable) — donne un nom, une piste, un signalement, sans révéler le secret lui-même. (2) **Informateur de rue** (barbier, tavernier, artisan, docker) — donne une adresse, un lieu, une indication de mouvement. (3) **Source directe** (témoin caché, complice repentant, victime) — donne la preuve, le nom, le mécanisme. Entre chaque entretien : un intermède de déplacement (traversée de la ville, changement de quartier) qui sert de respiration et de digestion pour le lecteur. Fréquence : 1 à 2 occurrences par roman d'enquête ; ne pas utiliser pour les entretiens secondaires.
